...
int main(int argc, char **argv)
{
    ...
    FILE *f_2sat_start = fopen("results_2SAT.csv", "w");
    fprintf(f_2sat_start, "pigeons,time\n");
    fclose(f_2sat_start);

    FILE *f_dpll = fopen("results_dpll.csv", "a");
    FILE *f_gwsat = fopen("results_gwsat.csv", "a");
    FILE *f_2sat = fopen("results_2SAT.csv", "a");

    for (int iterator = 1; iterator <= 5; iterator++)
    {
        char file_str[1024];
        sprintf(file_str, "cnf_files/2SAT%d.cnf", iterator);
        ...
        struct timespec start2SAT, end2SAT;

        clock_gettime(CLOCK_MONOTONIC, &start2SAT);

        int is_sat2SAT = GWSAT(number_of_variables, number_of_clauses, clauses, 500, 3 * number_of_variables, 0.6);
        printf("2SAT %d, %d\n", iterator, is_sat2SAT);

        clock_gettime(CLOCK_MONOTONIC, &end2SAT);

        double elapsed_2SAT = (end2SAT.tv_sec - start2SAT.tv_sec) + (end2SAT.tv_nsec - start2SAT.tv_nsec) / 1e9;

        printf("2SAT %d: (%.6f Sekunden)\n", iterator, elapsed_2SAT);
